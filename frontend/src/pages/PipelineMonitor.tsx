...
export function PipelineMonitor() {
  ...
  const [queueDetails, setQueueDetails] = useState(defaultQueueDetails)
  const [retryStats, setRetryStats] = useState(defaultRetryStats)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (data) {
        if (data.pipelineStages?.length) setStages(data.pipelineStages)
        if (data.events?.length) setEvents(data.events)
        if (data.queueDepth != null) setQueueDepth(data.queueDepth)
        if (data.queueDetails?.length) setQueueDetails(data.queueDetails)
        if (data.retryStats?.length) setRetryStats(data.retryStats)
      }
    } catch {
      // fallback to defaults retained
    } finally {
      setLoading(false)
    }
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div className="xeno-card">
          ...
          <div style={{ padding: 20, textAlign: 'center' }}>
            <div style={{ fontSize: 48, fontWeight: 700, color: '#14b8a6', marginBottom: 8 }}>{queueDepth}</div>
            <div style={{ fontSize: 14, color: '#64748b', marginBottom: 24 }}>Messages in Queue</div>
            <div style={{ display: 'flex', gap: 12 }}>
              {queueDetails.map((q, i) => (
